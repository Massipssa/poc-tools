...
if __name__ == "__main__":
    ...
    """
    - catalog name : demo
    - catalog type: hadoop (iceberg tables will be stored in filesystem)
    - warehouse
    """

    """
    org.apache.iceberg.spark.SparkCatalog: supports hive or hadoop (filesystem) as catalog
    org.apache.iceberg.spark.SparkCatalogSession: more specialized (to manage non-iceberg table alongside iceberg 
    tables
    spark.sql.catalog.custom_catalog.catalog-impl: to use custom implementation catalog like Glue
    """
    conf = SparkConf() \
